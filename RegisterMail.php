...
function SendMail($Gmail_name){
    ...
    $mail->Body = "親愛的 ".$Gmail_name."，您好：<br /><br />歡迎您註冊promdream請先點擊激活您的帳號，在審核完畢後我們將會寄信通知您!<br /> <a href='http://localhost/promdream/active.php?verify=".$token."' target='_blank'>http://localhost/promdream/active.php?verify=".$token."</a><br/>如果以上連結無法點擊，請將它複製到你的瀏覽器網址欄中進入訪問，該連結15分鐘內有效。"; //郵件內容
    $mail->IsHTML(true);                             //郵件內容為html
    $mail->AddAddress($email);            //收件者郵件及名稱
    $mail->Send();
}
SendMail($username);
echo "<script>location.href = 'http://localhost/promdream/EmailVerifyResult.html'</script>";
?>